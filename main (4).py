# PartyRadar — Render-ready main.py (PTB 20.8 compatible)
# - Uses Application.run_polling() (no deprecated Updater API)
# - JobQueue handles expiry/notifications
# - CryptoCloud payments (invoice link)
# - JSON storage next to main.py
# - Categories with lifetimes, push before expiry
# - Search by geolocation, banners, TOP promotion
# - Safe for Render cold starts
...
def get_event(eid: str) -> Optional[Event]:
    ...
    return Event(**info) if info else None
...
def list_events(include_hidden=False) -> List[Event]:
    ...
    res.sort(key=lambda e: (
        0 if (e.top_until and datetime.fromisoformat(e.top_until) > now) else 1,
        e.created_at
    ))
    return res
...
def get_user(uid: int) -> User:
    ...
    return User(id=uid) if not info else User(**info)
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Как пользоваться:\n"
        "• «➕ Создать событие» — заполни шаги.\n"
        "• «🔎 Поиск рядом» — пришли локацию (радиус 10 км).\n"
        "• Продвижение: /top.\n"
        "Сроки жизни: вечеринки/маркет — 36 ч, жильё — 48 ч, знакомства/попутчики — без ограничения."
    )
...
async def my_events(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not events:
        ...
        return
    lines = [f"• [{e.id}] {e.category} — {e.date} {e.time} — {'скрыто' if e.hidden else 'активно'}" for e in events]
    await update.message.reply_text("\n".join(lines))
...
async def ask_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Дата события (YYYY-MM-DD):")
    return ASK_DATE
...
async def ask_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["contact"] = "" if contact == "-" else contact
    await update.message.reply_text(
        "Отправь локацию (📍 геопозицию) или введи адрес текстом. Можно выбрать точку на карте (скрепка → Location)."
    )
    return ASK_LOCATION
...
async def ask_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.location:
        ...
        context.user_data["address"] = (update.message.text or "").strip()[:200]
...
async def finalize_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        await update.message.reply_text("Пришли фото или '-' для пропуска.")
        return ASK_PHOTO
...
async def search_nearby(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Пришли свою локацию (📍) — покажу события в радиусе 10 км.",
...
async def handle_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [f"• {e.category} — {e.date} {e.time} ({d:.1f} км)\n{e.description[:120]}" for d, e in nearby[:20]]
    await update.message.reply_text("\n\n".join(lines))
...
async def handle_promote_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Ссылка на оплату готова. После оплаты статус обновится автоматически.", reply_markup=kb)
    context.user_data.pop("await_promote_id", None)

# ---- Banners ----
async def banners(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db = _read(BANNERS_FILE) or {}
    items = db.get("banners") or []
    if not items:
        await update.message.reply_text("Баннеров пока нет.", reply_markup=MAIN_KB)
        return
    media = [InputMediaPhoto(b["photo"], caption=b.get("title","")) for b in items[:10] if b.get("photo")]
    if media:
        await update.message.reply_media_group(media)
    else:
        await update.message.reply_text("\n".join([b.get("title","") for b in items]))

# ---- JobQueue task: expiry + notifications ----
async def expiry_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for eid, ed in list((db.get("events") or {}).items()):
        ...
        if ev.expires_at and not ev.hidden:
            ...
            lead = exp_dt - now
            if timedelta(hours=PUSH_LEAD_HOURS) >= lead > timedelta(hours=PUSH_LEAD_HOURS - 0.1):
                to_notify.append((ev.user_id, f"⏰ Событие {ev.id} истекает через {PUSH_LEAD_HOURS} ч. Продлить? /top"))
            if now >= exp_dt:
                ev.hidden = True
...
def build_app() -> Application:
    ...
    conv = ConversationHandler(
        ...
        states={
            ASK_CAT: [MessageHandler(filters.TEXT & ~filters.COMMAND, ask_desc)],
            ...
            ASK_TIME: [MessageHandler(filters.TEXT & ~filters.COMMAND, ask_contact)],
            ASK_CONTACT: [MessageHandler(filters.TEXT & ~filters.COMMAND, ask_location)],
            ASK_LOCATION: [
    ...
    # Job: check expiry every 60s (first run after 5s)
    app.job_queue.run_repeating(expiry_job, interval=60, first=5)

    return app

def main():
    app = build_app()
    # Small delay so Render cold start settles
    async def _delayed():
        await asyncio.sleep(2)
        await app.initialize()
        await app.start()
        await app.run_polling(close_loop=False, drop_pending_updates=True)
    asyncio.run(_delayed())
