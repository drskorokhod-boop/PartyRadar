...
@dp.message(AddEvent.media, F.text == "Пропустить")
async def ev_media_skip(m: Message, state: FSMContext):
    # Пользователь решил пропустить медиа и перейти сразу к геолокации
    await m.answer(
        "📍 Отправь геолокацию для события.
"
        "Скрепка → Геопозиция → точка на карте.",
...
async def ev_lifetime(m: Message, state: FSMContext):
    ...
    desc = (
        ...
        "⭐ ТОП-продвижение — закрепление события выше остальных.\n"
        "📣 Push-рассылка — уведомления пользователям в радиусе.\n\n"
...
"
    ...
    # Оплата прошла — публикуем событие
    ev = await publish_event(m, data, hours)
    await m.answer("☑️ Оплата подтверждена! Событие опубликовано.")

    # Покажем карточку события пользователю
    try:
        await send_event_media(m.chat.id, ev)
    except Exception:
        # fallback — текстовая карточка
