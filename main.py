...
async def send_logo_then_welcome(m: Message):
    ...
    await m.answer(
        ...
        "📍 Поиск людей и событий по геолокации.\n\n"
        "📅 ВАЖНО: дату/время и цену ты указываешь прямо в описании события.\n"
        "Отдельного шага с датой больше нет — всё максимально просто.\n\n"
        "👇 Выбирай действие:",
...
async def support(m: Message):
    await m.answer(
        "💬 Идеи, предложения, реклама:\n"
        "👉 <b>@drscorohod</b>\n\n"
        "Стараемся отвечать всем ❤️",
        reply_markup=kb_main()
    )
...
async def ev_title(m: Message, state: FSMContext):
    ...
    await m.answer(
        "🧾 Введи <b>описание</b> события.\n\n"
        "👉 Сюда же укажи:\n"
        "• дату и время (например, 25.10 в 19:30)\n"
        "• район / место\n"
        "• цену / стоимость участия (если есть)\n"
        "• формат и важные условия.\n\n"
        "Отдельного шага с датой нет — всё пишем в описании.",
...
async def ev_cat(m: Message, state: FSMContext):
    ...
    if cat in MARKET_CATS:
        ...
        return await m.answer(
            ...
            "Примеры: <b>150€</b>, <b>200$</b>, <b>5000₽</b>, <b>договорная</b>.\n\n"
            "Срок актуальности и детали сделки укажи в описании.",
            reply_markup=kb_back()
        )
    ...
    if cat in ("💼 Ищу работу", "🧑‍💼 Предлагаю работу"):
        ...
        return await m.answer(
            "💼 Рубрика «Работа».\n"
            "В описании можно указать:\n"
            "• формат (офис/удалёнка)\n"
            "• график\n"
            "• зарплату/вилку\n"
            "• дату начала и требования.\n\n"
            "Теперь прикрепи до 3 фото/видео или отправь геолокацию.",
    ...
    if cat == "✨ Покажи себя":
        ...
        await state.set_state(AddEvent.media)
        return await m.answer(
            "✨ Рубрика самопрезентаций и знакомств.\n"
            "В описании укажи город/район, возраст, чем занимаешься и что ищешь.\n"
            "Если предлагаешь услуги — цену тоже можно написать там.\n\n"
            "Прикрепи до 3 фото/видео или отправь геолокацию.",
            reply_markup=kb_media_step()
        )
    ...
    if cat == "🔍 Ищу тебя":
        ...
        return await m.answer(
            ...
            "В описании напиши, кого ищешь, где и когда видел, как с тобой связаться.\n\n"
            "Можно прикрепить до 3 фото/видео или отправить геолокацию.",
            reply_markup=kb_media_step()
        )
    ...
    if cat == "🎊 Поздравления":
        ...
        await state.set_state(AddEvent.media)
        return await m.answer(
            "🎊 Рубрика поздравлений и хороших новостей.\n"
            "В описании можешь указать повод, дату события и кому адресовано.\n\n"
            "Прикрепи до 3 фото/видео или отправь геолокацию.",
            reply_markup=kb_media_step()
        )
    ...
    await m.answer(
        ...
        "Напоминание: дату/время и цену участия укажи в описании.",
        reply_markup=kb_media_step()
    )
...
async def ev_dt(m: Message, state: FSMContext):
    ...
    await m.answer(
        ...
        "📍 Скрепка → Геопозиция → точка на карте.\n"
        "⚠ Аудио и кружки не поддерживаются.\n\n"
        "Срок актуальности и условия сделки уже должны быть в описании.",
        reply_markup=kb_media_step()
    )
...
async def ev_media_back(m: Message, state: FSMContext):
    ...
    if files:
        await m.answer("Чтобы изменить медиа, проще пересоздать объявление заново 🙏", reply_markup=kb_media_step())
        return
...
async def ev_contact(m: Message, state: FSMContext):
    if m.text == "⬅ Назад":
        await state.set_state(AddEvent.media)
        return await m.answer("Вернулись к медиа:", reply_markup=kb_media_step())
...
def _check_text_moderation(text: str) -> Tuple[bool, Optional[str]]:
    ...
    for group, words in FORBIDDEN_KEYWORDS_GROUPS.items():
        for w in words:
            if w in t:
                if group == "adult":
                    return False, "Объявление похоже на 18+ контент, такое размещать нельзя."
                if group == "drugs":
...
async def ev_lifetime(m: Message, state: FSMContext):
    ...
    if hours == 24:
        ...
        return await m.answer(
            ...
            "Хочешь усилить охват? Можешь включить ТОП, Push или баннер:",
...
async def ev_upsell(m: Message, state: FSMContext):
    ...
    if txt == "🌍 Оставить без доп.опций":
        ...
        return await m.answer(
            ...
            "Ты всегда можешь вернуться и докупить ТОП, Push или баннер позже.",
            reply_markup=kb_main()
        )
...
async def ev_opt_back(m: Message, state: FSMContext):
    await state.set_state(AddEvent.upsell)
    await m.answer("Выбери дополнительную опцию:", reply_markup=kb_upsell())
...
async def ev_opt_router(m: Message, state: FSMContext):
    ...
    if txt == "✅ Я оплатил":
        ...
        if opt_type == "top":
            ...
            _save_events(events)
            await state.clear()
            return await m.answer(f"🎉 ТОП активирован на {days} дней!", reply_markup=kb_main())

        if opt_type == "push":
            sent = await send_push_for_event(target)
            await state.clear()
            return await m.answer(f"📣 PUSH-рассылка отправлена. Получателей: {sent}.", reply_markup=kb_main())
...
async def banner_paid(m: Message, state: FSMContext):
    ...
    await state.clear()
    await m.answer("✅ Баннер активирован и будет показан пользователям в регионе.", reply_markup=kb_main())
...
async def search_start(m: Message, state: FSMContext):
    ...
    await m.answer(
        "Что ищем?\n"
        "🔎 Все события — встречи, вечеринки, спорт и т.п.\n"
        "🛒 Маркет — куплю/продам.\n"
