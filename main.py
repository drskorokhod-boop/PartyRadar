...
def get_user_lang(user_id: int) -> str:
    """
    Возвращаем язык пользователя. По умолчанию 'ru'.
    """
    users = _load_users()
...
def set_user_lang(user_id: int, lang: str):
    """
    Сохраняем язык пользователя в users_store.
    """
    users = _load_users()
    u = users.get(str(user_id)) or {}
    u["lang"] = lang
...
def kb_main():
    # По умолчанию русская раскладка, если язык пользователя неизвестен
    return kb_main_lang("ru")


def kb_main_for_user(user_id: int):
    lang = get_user_lang(user_id)
    return kb_main_lang(lang)
...
async def send_event_media(chat_id: int, ev: dict, with_distance: Optional[float] = None):
    ...
    if len(media) > 1:
        ...
        for i, f in enumerate(media):
            # для альбома не дублируем текст в подписи, показываем его только в отдельном сообщении с кнопками
            caption = None
            if f["type"] == "photo":
        ...
        # после группы отдельным сообщением отправляем текст и кнопки (карта, избранное, удалить)
        if ikb:
...
async def send_logo_then_welcome(m: Message):
    ...
    await m.answer(
        ...
        "• 🖼 Баннеры над лентой\n\n"
        "Выбирай, что делаем 👇",
        reply_markup=kb_main_for_user(m.from_user.id)
    )
...
async def choose_language(m: Message):
    txt = m.text or ""
    if txt == "🇷🇺 Русский":
        set_user_lang(m.from_user.id, "ru")
        await m.answer(
            "Язык: Русский ✅\n\n"
            "Добро пожаловать в PartyRadar!\n"
            "Используй меню ниже, чтобы найти события или создать своё.",
            reply_markup=kb_main_for_user(m.from_user.id)
        )
    elif txt == "🇬🇧 English":
        set_user_lang(m.from_user.id, "en")
        await m.answer(
            "Language: English ✅\n\n"
            "Welcome to PartyRadar!\n"
            "Use the menu below to find events or create your own.",
            reply_markup=kb_main_for_user(m.from_user.id)
        )
    try:
...
async def create_event_start(m: Message, state: FSMContext):
    ...
    if lang == "en":
        await m.answer(
            ...
            "First, write a <b>short title</b> so it's immediately clear what it is about.",
...
async def ev_title(m: Message, state: FSMContext):
    if m.text in ["⬅ Назад", "⬅ Back"]:
        ...
        return await m.answer(
            "Окей, вернулись в главное меню 👋" if get_user_lang(m.from_user.id) != "en" else "Okay, back to the main menu 👋",
            reply_markup=kb_main_for_user(m.from_user.id)
        )
    ...
    if lang == "en":
        await m.answer(
            "✏️ Now enter a <b>detailed description</b> of your event.\n\n"
            "Be sure to include:\n"
            "- date and time (for example: 25.10 at 19:30)\n"
            "- place / location\n"
            "- price / entry fee (if any)\n"
            "- format and important conditions (dress code, limits, etc.)\n\n"
            "⚠️ Date and time will not be asked again — write them directly in the text.\n\n"
            "If something goes wrong — open the menu on the left of the input field and press “Restart bot”.",
            reply_markup=kb_back(),
        )
    else:
        await m.answer(
...
async def ev_desc(m: Message, state: FSMContext):
    if m.text in ["⬅ Назад", "⬅ Back"]:
        ...
        lang = get_user_lang(m.from_user.id)
        if lang == "en":
            return await m.answer("✏️ Enter the event title:", reply_markup=kb_back())
        else:
    ...
    if lang == "en":
        await m.answer(
            ...
            "📝 If something goes wrong — open the menu on the left of the input field and choose “Restart bot”.",
...
async def ev_cat(m: Message, state: FSMContext):
    if m.text in ["⬅ Назад", "⬅ Back"]:
        ...
        lang = get_user_lang(m.from_user.id)
        if lang == "en":
            return await m.answer("🧾 Enter the event description:", reply_markup=kb_back())
        else:
    ...
    if cat in MARKET_CATS:
        ...
        if lang == "en":
            return await m.answer(
                "💵 Enter the price (you can add a currency symbol):\n"
                "For example: <b>150€</b>, <b>200$</b>, <b>5000₽</b>, <b>negotiable</b>.\n\n"
                "❗ Write how long the offer is valid and deal details in the description.",
                reply_markup=kb_back()
            )
        else:
            return await m.answer(
    ...
    if cat in ("💼 Ищу работу", "🧑‍💼 Предлагаю работу"):
        ...
        if lang == "en":
            return await m.answer(
                "💼 Job section.\n\n"
                "Your description should already include:\n"
                "• format (office / remote)\n"
                "• schedule\n"
                "• salary / range\n"
                "• requirements and start date.\n\n"
                "📎 Now media step. Attach up to 3 photos/videos or send a location.\n"
                "❗ If you plan to use a banner later — <b>you must upload media at this step</b>.",
    ...
    if cat == "✨ Покажи себя":
        ...
        if lang == "en":
            return await m.answer(
                "✨ Here you can tell about yourself or your project.\n\n"
                "Your description should already include:\n"
                "• city / area\n"
                "• age (if relevant)\n"
                "• who you are and what you're looking for\n"
                "• price for services (if any).\n\n"
                "📎 Now attach up to 3 photos/videos or send a location.\n"
                "❗ If you want to launch a banner later — you need to upload media <b>right now</b>.",
                reply_markup=kb_media_step()
            )
        else:
            return await m.answer(
    ...
    if cat == "🐾 Ищу тебя":
        ...
        if lang == "en":
            return await m.answer(
                "👀 Search for people or pets.\n\n"
                "Your description should already include:\n"
                "• who you are looking for\n"
                "• where and when you saw them last\n"
                "• how to contact you\n\n"
                "📎 Attach up to 3 photos/videos (if any) or send the location of the place.\n\n"
                "ℹ️ If something goes wrong — open the menu on the left of the input field and choose “Restart bot”.",
                reply_markup=kb_media_step(),
            )
        else:
            return await m.answer(
    ...
    # Поздравления
        ...
        if lang == "en":
            return await m.answer(
                "🎉 Congratulations and nice news.\n\n"
                "In the description you can specify the occasion, date and who it is addressed to.\n\n"
                "📎 Attach up to 3 photos/videos or send location (if appropriate).\n\n"
                "ℹ️ If something goes wrong — open the menu on the left of the input field and choose “Restart bot”.",
                reply_markup=kb_media_step(),
            )
        else:
    ...
    # Остальные — просто события
        ...
        if lang == "en":
            return await m.answer(
                "📸 Media step!\n\n"
                "Attach up to 3 photos/videos (atmosphere, place, flyer)\n"
                "or send a location.\n\n"
                "If you want to launch a <b>banner</b> later — you need to upload media <b>right now</b> for this event.\n"
                "⚠️ Audio and video notes are not supported.\n\n"
                "ℹ️ If something goes wrong — open the menu on the left of the input field and choose “Restart bot”.",
                reply_markup=kb_media_step(),
            )
        else:
...
async def ev_lifetime(m: Message, state: FSMContext):
    ...
    if hours == 24:
        ...
        try:
            ...
            logging.exception(f"Ошибка отправки медиа события: {e}")
            await m.answer("Событие опубликовано, но медиа не получилось отправить автоматически.")
...
@dp.message(AddEvent.payment, F.text.in_(["💳 Оплата картой (BitPapa)", "💳 Pay by card (BitPapa)"]))
...
@dp.message(AddEvent.payment, F.text.in_(["🪙 Оплата криптовалютой (USDT)", "🪙 Pay with crypto (USDT)"]))
...
@dp.message(AddEvent.payment, F.text.in_(["💳 Получить ссылку на оплату", "💳 Get payment link"]))
...
@dp.message(AddEvent.payment, F.text.in_(["✅ Я оплатил", "✅ I paid"]))
...
async def ev_upsell(m: Message, state: FSMContext):
    ...
    if txt == "⬅ Назад":
        await state.clear()
        return await m.answer("Главное меню:", reply_markup=kb_main_for_user(m.from_user.id))

    if txt in ("🌍 Оставить без доп.опций", "🌍 No extra options"):
        await state.clear()
        return await m.answer(
    ...
    if txt in ("⭐ Продвижение ТОП", "⭐ TOP promotion"):
        await m.answer(
    ...
    # Push
    if txt in ("📣 Push-рассылка (30 км)", "📣 Push campaign (30 km)"):
        events = _load_events()
        user_events = [e for e in events if int(e.get("author", 0)) == int(m.from_user.id)]
        if not user_events:
            await state.clear()
            return await m.answer("❌ У тебя пока нет опубликованных событий.", reply_markup=kb_main_for_user(m.from_user.id))
   ...
   # Баннер
    if txt in ("🖼 Баннер (премиум)", "🖼 Banner (premium)"):
        events = _load_events()
        user_events = [e for e in events if int(e.get("author", 0)) == int(m.from_user.id)]
        if not user_events:
            await state.clear()
            return await m.answer("❌ У тебя пока нет событий для баннера.", reply_markup=kb_main_for_user(m.from_user.id))
...
async def ev_upsell_more(m: Message, state: FSMContext):
    ...
    if txt in ("🏠 Главное меню", "Главное меню"):
        await state.clear()
        return await m.answer("Возврат в главное меню 👇", reply_markup=kb_main_for_user(m.from_user.id))
...
async def ev_opt_router(m: Message, state: FSMContext):
    ...
    if txt.startswith("⭐ "):
        ...
        if not user_events:
            await state.clear()
            return await m.answer("❌ У тебя нет событий для ТОП.", reply_markup=kb_main_for_user(m.from_user.id))
    ...
    if txt == "✅ Я оплатил":
        ...
        if not target:
            await state.clear()
            return await m.answer("❌ Событие не найдено.", reply_markup=kb_main_for_user(m.from_user.id))
        ...
        if opt_type == "push":
            ...
            if lang == "en":
                text = (
                    f"📣 Push campaign has been sent. Reach: {sent} users.\n"
                    "Do you want to add another option to this event?"
                )
            else:
                text = (
...
@dp.message(AddBanner.payment, F.text.in_(["💳 Оплата картой (BitPapa)", "💳 Pay by card (BitPapa)"]))
...
@dp.message(AddBanner.payment, F.text.in_(["🪙 Оплата криптовалютой (USDT)", "🪙 Pay with crypto (USDT)"]))
...
@dp.message(AddBanner.payment, F.text.in_(["💳 Получить ссылку на оплату", "💳 Get payment link"]))
...
@dp.message(AddBanner.payment, F.text.in_(["✅ Я оплатил", "✅ I paid"]))
async def banner_paid(m: Message, state: FSMContext):
    ...
    media = d.get("b_media")
    if not media:
        return await m.answer("❌ Медиа не найдено. Начни заново.", reply_markup=kb_main_for_user(m.from_user.id))
...
async def search_menu_router(m: Message, state: FSMContext):
    ...
    if not category_filter:
        return await m.answer("Выбери один из вариантов:", reply_markup=kb_main_for_user(m.from_user.id))
...
async def show_favorites(m: Message):
    ...
    if not fav_ids:
        return await m.answer("У тебя пока нет избранных событий ⭐", reply_markup=kb_main_for_user(m.from_user.id))
