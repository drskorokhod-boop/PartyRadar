...
def init_db():
    """Инициализация SQLite: таблица пользователей для хранения языка и активности."""
...
def set_user_lang(m: Message, lang: str):
    """Сохранить язык пользователя в SQLite и обновить базовую информацию."""
    try:
    ...
    try:
        ...
        cur.execute(
            ...
            (
                uid,
                (m.from_user.username or "") if m.from_user else "",
                (m.from_user.first_name or "") if m.from_user else "",
                (m.from_user.last_name or "") if m.from_user else "",
                lang,
                now,
                now,
            ),
...
def get_user_lang(user_id: int) -> str:
    """Получить язык пользователя из SQLite. По умолчанию — 'ru'."""
    try:
        uid = int(user_id)
    except Exception:
        return "ru"
...
def kb_main(lang: str = "ru"):
    if lang == "en":
        return ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="📍 Find events nearby")],
...
def kb_main_for(m: Message) -> ReplyKeyboardMarkup:
    lang = get_user_lang(m.from_user.id if m.from_user else 0)
    return kb_main(lang)


def kb_back(lang: str = "ru"):
    txt = "⬅ Назад" if lang == "ru" else "⬅ Back"
    return ReplyKeyboardMarkup(
...
def kb_media_step(lang: str = "ru"):
    back_txt = "⬅ Назад" if lang == "ru" else "⬅ Back"
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text=back_txt)]],
        resize_keyboard=True
    )


def kb_categories(lang: str = "ru"):
    if lang == "en":
        return ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="🎉 Party"), KeyboardButton(text="💬 Date")],
                [KeyboardButton(text="🧠 Meet by interests"), KeyboardButton(text="⚽ Activity/Sport")],
                [KeyboardButton(text="🛒 Buy"), KeyboardButton(text="💰 Sell")],
                [KeyboardButton(text="💼 Looking for a job"), KeyboardButton(text="🧑‍💼 Offering a job")],
                [KeyboardButton(text="✨ Show yourself"), KeyboardButton(text="🔍 Looking for you")],
                [KeyboardButton(text="🎊 Greetings/Congrats"), KeyboardButton(text="🧭 Other")],
...
def kb_lifetime(lang: str = "ru"):
    if lang == "en":
        return ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="🕐 1 day (free)")],
                [KeyboardButton(text="⏱ 3 days — $0.8"), KeyboardButton(text="⏱ 7 days — $1.5")],
                [KeyboardButton(text="⏱ 30 days — $3.0")],
                [KeyboardButton(text="⬅ Back")],
            ],
            resize_keyboard=True
        )
...
def kb_payment(lang: str = "ru"):
    if lang == "en":
        return ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="💳 Get payment link")],
...
def kb_payment_method(lang: str = "ru"):
    if lang == "en":
        return ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="💳 Pay by card (BitPapa)")],
                [KeyboardButton(text="🪙 Pay with crypto (USDT)")],
                [KeyboardButton(text="⬅ Back")],
            ],
            resize_keyboard=True
        )
...
def kb_upsell(lang: str = "ru"):
    if lang == "en":
        return ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="⭐ TOP promotion")],
                [KeyboardButton(text="📣 Push broadcast (30 km)")],
                [KeyboardButton(text="🖼 Banner (premium)")],
                [KeyboardButton(text="🌍 No extra options")],
                [KeyboardButton(text="⬅ Back")],
            ],
            resize_keyboard=True
        )
...
def kb_upsell_more(lang: str = "ru"):
    if lang == "en":
        return ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="➕ Add another option")],
...
def kb_top_duration(lang: str = "ru"):
    if lang == "en":
        return ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="⭐ 1 day — $1"), KeyboardButton(text="⭐ 3 days — $2")],
                [KeyboardButton(text="⭐ 7 days — $3"), KeyboardButton(text="⭐ 15 days — $5")],
                [KeyboardButton(text="⭐ 30 days — $8")],
                [KeyboardButton(text="⬅ Back")],
            ],
            resize_keyboard=True
        )
...
def kb_banner_duration(lang: str = "ru"):
    if lang == "en":
        return ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="📅 1 day — $3"), KeyboardButton(text="📅 3 days — $7")],
                [KeyboardButton(text="📅 7 days — $12"), KeyboardButton(text="📅 15 days — $18")],
                [KeyboardButton(text="📅 30 days — $30")],
                [KeyboardButton(text="⬅ Back")],
            ],
            resize_keyboard=True
        )
...
async def send_logo_then_welcome(m: Message, lang: str = "ru"):
    logo_path = None
    ...
    if lang == "en":
        text_welcome = (
            "⚡️ <b>PartyRadar</b>\n\n"
            "Here you can find events and create your own — fast and easy.\n\n"
            "Features:\n"
    ...
    await m.answer(text_welcome, reply_markup=kb_main(lang))
...
async def start_cmd(m: Message, state: FSMContext):
    ...
    # Проверяем, выбран ли язык ранее
    lang = None
    try:
        uid = int(m.from_user.id)
    except Exception:
    ...
    if not lang:
        ...
        await state.set_state(Lang.choose)
        return await m.answer(
            "Выбери язык / Choose your language 👇",
            reply_markup=kb_language()
        )

    await send_logo_then_welcome(m, lang)
    # Попробуем показать баннер рядом (если есть сохранённая геолокация)
    await show_nearby_banner_for_user(m)



@dp.message(StateFilter(Lang.choose))
async def choose_language(m: Message, state: FSMContext):
    txt = (m.text or "").lower()
    if "рус" in txt:
        lang = "ru"
    elif "eng" in txt or "англ" in txt or "english" in txt:
        lang = "en"
    else:
        return await m.answer(
            "Пожалуйста, выбери язык кнопками ниже.\n"
            "Please choose a language using the buttons.",
            reply_markup=kb_language()
        )

    set_user_lang(m, lang)
    await state.clear()
    await send_logo_then_welcome(m, lang)
    await show_nearby_banner_for_user(m)
...
@dp.message(F.text.in_(["📩 Связаться с нами", "📩 Contact us"]))
async def support(m: Message):
    await m.answer(
        "💬 Идеи, баги, реклама, коллаборации:\n"
        "👉 <b>@drscorohod</b>\n\n"
        "Пишешь — читаем. Не всегда мгновенно, но живые люди 🙂",
        reply_markup=kb_main_for(m)
    )


# ===================== СОЗДАНИЕ СОБЫТИЙ =====================

@dp.message(F.text.in_(["➕ Создать событие", "➕ Create event"]))
async def create_event_start(m: Message, state: FSMContext):
    ...
    await m.answer(
        ...
        reply_markup=kb_back(get_user_lang(m.from_user.id if m.from_user else 0))
    )


@dp.message(AddEvent.title)
async def ev_title(m: Message, state: FSMContext):
    if m.text in ["⬅ Назад", "⬅ Back"]:
        await state.clear()
        return await m.answer("Окей, вернулись в главное меню 👋", reply_markup=kb_main_for(m))
...
async def ev_desc(m: Message, state: FSMContext):
    if m.text in ["⬅ Назад", "⬅ Back"]:
        await state.set_state(AddEvent.title)
        return await m.answer("✏️ Напиши заголовок события:", reply_markup=kb_back(get_user_lang(m.from_user.id if m.from_user else 0)))
    ...
    await m.answer(
        ...
        reply_markup=kb_categories(get_user_lang(m.from_user.id if m.from_user else 0))
    )
...
async def ev_cat(m: Message, state: FSMContext):
    if m.text in ["⬅ Назад", "⬅ Back"]:
        await state.set_state(AddEvent.description)
        return await m.answer("🧾 Введи описание события:", reply_markup=kb_back(get_user_lang(m.from_user.id if m.from_user else 0)))
    ...
    if cat in ("💼 Ищу работу", "🧑‍💼 Предлагаю работу"):
        ...
        return await m.answer(
            ...
            reply_markup=kb_media_step(get_user_lang(m.from_user.id if m.from_user else 0))
        )
...
async def ev_dt(m: Message, state: FSMContext):
    ...
    if m.text in ["⬅ Назад", "⬅ Back"]:
        await state.set_state(AddEvent.category)
        return await m.answer("📂 Вернулись к выбору категории:", reply_markup=kb_categories(get_user_lang(m.from_user.id if m.from_user else 0)))
...
async def ev_media(m: Message, state: FSMContext):
    ...
    if len(files) >= MAX_MEDIA:
        return await m.answer("⚠ Уже 3 файла. Теперь отправь геолокацию.", reply_markup=kb_media_step(get_user_lang(m.from_user.id if m.from_user else 0)))
...
@dp.message(AddEvent.media, F.text.in_(["⬅ Назад", "⬅ Back"]))
async def ev_media_back(m: Message, state: FSMContext):
    ...
    if category in MARKET_CATS:
        await state.set_state(AddEvent.dt)
        return await m.answer("💵 Укажи цену:", reply_markup=kb_back(get_user_lang(m.from_user.id if m.from_user else 0)))

    await state.set_state(AddEvent.category)
    await m.answer("🧭 Вернулись к выбору категории:", reply_markup=kb_categories(get_user_lang(m.from_user.id if m.from_user else 0)))
...
async def ev_contact(m: Message, state: FSMContext):
    ...
    await m.answer(
        ...
        reply_markup=kb_lifetime(get_user_lang(m.from_user.id if m.from_user else 0))
    )
...
FORBIDDEN_KEYWORDS_GROUPS = {
    "adult": [
        "интим", "интимка", "эскорт", "эскортница", "проститут", "шлюха", "секс услуги", "sex услуги",
        "sex chat", "секс чат", "минет", "оральный секс", "onlyfans", "only fans", "онлифанс",
        "эротика", "эротический", "nudes", "nude", "18+"
    ],
    "drugs": [
        "закладка", "закладки", "закладчик", "наркотик", "наркота", "метамфетамин", "амфетамин",
        "героин", "кокаин", "марихуана", "шишки", "спайс", "миксы", "курительная смесь",
        "меф", "мефедрон", "mdma", "экстази", "экстази", "скорость", "спиды"
    ],
    "weapons": [
        "оружие", "пистолет", "автомат калашникова", "ak-47", "ak47",
        "нож-бабочка", "куплю гранату", "продам гранату", "продам оружие",
        "ствол", "стволы", "пушка", "глок", "макаров", "травмат", "резинострел"
    ],
    "gambling": [
        "ставки на спорт", "казино", "1xbet", "1хбет", "букмекерская контора",
        "игровые автоматы", "слоты", "рулетка", "казик", "беттинг", "ставочки", "слотики"
    ],
    "fraud": [
        "легкий заработок без вложений", "быстрый заработок", "доход 1000$ в день",
        "пирамида", "финансовая пирамида", "инвестиции без риска",
        "слив схем", "слив схемы", "дропы", "обнал", "обналичка",
        "быстрые деньги", "лёгкие деньги", "легкие деньги", "лохотрон", "развести на деньги"
    ],
}
...
async def ev_lifetime(m: Message, state: FSMContext):
    ...
    if m.text in ["⬅ Назад", "⬅ Back"]:
        await state.set_state(AddEvent.contact)
        back_text = "☎ Укажи контакт или напиши «Пропустить»." if lang == "ru" else "☎ Please enter a contact or type “Skip”."
        return await m.answer(back_text, reply_markup=kb_back(lang))

    if m.text not in LIFETIME_OPTIONS:
        msg = "Выбери срок из списка:" if lang == "ru" else "Please choose a duration from the list:"
        return await m.answer(msg, reply_markup=kb_lifetime(lang))
    ...
    if hours == 24:
        if user_has_active_free_event(user_id, category):
            ...
            return await m.answer(
                ...
                reply_markup=kb_payment(get_user_lang(m.from_user.id if m.from_user else 0))
            )
        ...
        return await m.answer(
            ...
            reply_markup=kb_upsell(get_user_lang(m.from_user.id if m.from_user else 0))
        )
    ...
    await m.answer(
        ...
        reply_markup=kb_payment_method(get_user_lang(m.from_user.id if m.from_user else 0))
    )
...
async def ev_pay_method_card(m: Message, state: FSMContext):
    ...
    await m.answer(txt, reply_markup=kb_payment(get_user_lang(m.from_user.id if m.from_user else 0)))
...
async def ev_pay_get(m: Message, state: FSMContext):
    ...
    if not hours:
        return await m.answer("❌ Нет активного платного тарифа.", reply_markup=kb_payment(get_user_lang(m.from_user.id if m.from_user else 0)))
    ...
    if not link or not invoice_id:
        return await m.answer("⚠️ Не удалось получить ссылку на счёт. Проверь API ключ.", reply_markup=kb_payment(get_user_lang(m.from_user.id if m.from_user else 0)))
...
async def ev_pay_check(m: Message, state: FSMContext):
    ...
    if not invoice_uuid or not hours:
        return await m.answer("⚠️ Ошибка: не найден счёт или тариф.", reply_markup=kb_payment(get_user_lang(m.from_user.id if m.from_user else 0)))
...
@dp.message(AddEvent.payment, F.text.in_(["⬅ Назад", "⬅ Back"]))
async def ev_pay_back(m: Message, state: FSMContext):
    await state.set_state(AddEvent.lifetime)
    await m.answer("🔙 Вернулись к выбору срока:", reply_markup=kb_lifetime(get_user_lang(m.from_user.id if m.from_user else 0)))
...
async def ev_upsell(m: Message, state: FSMContext):
    ...
    # map English button texts to Russian internal values
    if txt == "⬅ Back":
        txt = "⬅ Назад"
    elif txt == "🌍 No extra options":
        txt = "🌍 Оставить без доп.опций"
    elif txt == "⭐ TOP promotion":
        txt = "⭐ Продвижение ТОП"
    elif txt == "📣 Push broadcast (30 km)":
        txt = "📣 Push-рассылка (30 км)"
    elif txt == "🖼 Banner (premium)":
        txt = "🖼 Баннер (премиум)"
    ...
    if txt == "⬅ Назад":
        ...
        return await m.answer(msg, reply_markup=kb_main_for(m))
    if txt == "🌍 Оставить без доп.опций":
        await state.clear()
        if lang == "ru":
            text = ("✔️ Готово! Событие опубликовано.\n"

                    "Если захочешь — всегда можно вернуться и докупить ТОП, Push или баннер.")
        else:
            text = ("✔️ Done! Your event has been published.\n"

                    "You can always come back later and buy TOP, Push or Banner.")
        return await m.answer(text, reply_markup=kb_main_for(m))
    if txt == "⭐ Продвижение ТОП":
        await m.answer(
            ...
            reply_markup=kb_top_duration(get_user_lang(m.from_user.id if m.from_user else 0))
        )
        ...
        return await m.answer("Выбери срок ТОП-продвижения:", reply_markup=kb_top_duration(get_user_lang(m.from_user.id if m.from_user else 0)))
    ...
    if txt == "📣 Push-рассылка (30 км)":
        ...
        if not user_events:
            await state.clear()
            return await m.answer("❌ У тебя пока нет опубликованных событий.", reply_markup=kb_main_for(m))
   ...
   # Баннер
    if txt == "🖼 Баннер (премиум)":
        ...
        if not user_events:
            await state.clear()
            return await m.answer("❌ У тебя пока нет событий для баннера.", reply_markup=kb_main_for(m))
        ...
        return await m.answer("Выбери срок показа баннера:", reply_markup=kb_banner_duration(get_user_lang(m.from_user.id if m.from_user else 0)))


@dp.message(StateFilter(AddEvent.pay_option), F.text.in_(["⬅ Назад", "⬅ Back"]))
async def ev_opt_back(m: Message, state: FSMContext):
    await state.set_state(AddEvent.upsell)
    await m.answer("Выбери дополнительную опцию:\n\n"
        "ℹ️ Если что-то пошло не так — нажми меню слева от строки ввода и выбери «Перезапустить бота».", reply_markup=kb_upsell(get_user_lang(m.from_user.id if m.from_user else 0)))
...
async def ev_upsell_more(m: Message, state: FSMContext):
    ...
    if txt == "➕ Добавить ещё опцию":
        await state.set_state(AddEvent.upsell)
        return await m.answer("Выберите дополнительную опцию для этого объявления:", reply_markup=kb_upsell(get_user_lang(m.from_user.id if m.from_user else 0)))

    if txt in ("🏠 Главное меню", "Главное меню"):
        await state.clear()
        return await m.answer("Возврат в главное меню 👇", reply_markup=kb_main_for(m))

    await m.answer("Выбери вариант из меню ниже 👇", reply_markup=kb_upsell_more(get_user_lang(m.from_user.id if m.from_user else 0)))
...
async def ev_opt_router(m: Message, state: FSMContext):
    ...
    if txt.startswith("⭐ "):
        try:
            ...
            return await m.answer("❌ Не понял срок. Выбери из меню.", reply_markup=kb_top_duration(get_user_lang(m.from_user.id if m.from_user else 0)))

        if days not in TOP_PRICES:
            return await m.answer("❌ Такого срока нет.", reply_markup=kb_top_duration(get_user_lang(m.from_user.id if m.from_user else 0)))

        events = _load_events()
        user_events = [e for e in events if int(e.get("author", 0)) == int(m.from_user.id)]
        if not user_events:
            await state.clear()
            return await m.answer("❌ У тебя нет событий для ТОП.", reply_markup=kb_main_for(m))
    ...
    if txt == "💳 Оплата картой (BitPapa)":
        ...
        return await m.answer(txt_help, reply_markup=kb_payment(get_user_lang(m.from_user.id if m.from_user else 0)))
    ...
    if txt == "💳 Получить ссылку на оплату":
        ...
        if opt_type not in ("top", "push") or not ev_id:
            return await m.answer("❌ Не выбрана опция.", reply_markup=kb_upsell(get_user_lang(m.from_user.id if m.from_user else 0)))
        ...
        if opt_type == "top":
            ...
            if not amount:
                return await m.answer("❌ Неверный срок ТОП.", reply_markup=kb_top_duration(get_user_lang(m.from_user.id if m.from_user else 0)))
        ...
        if not link or not invoice_id:
            return await m.answer("⚠️ Не удалось создать счёт.", reply_markup=kb_payment(get_user_lang(m.from_user.id if m.from_user else 0)))
    ...
    if txt == "✅ Я оплатил":
        ...
        if not invoice_uuid:
            return await m.answer("❌ Счёт не найден.", reply_markup=kb_payment(get_user_lang(m.from_user.id if m.from_user else 0)))

        paid = await cc_is_paid(invoice_uuid)
        if not paid:
            return await m.answer("❌ Оплата не найдена. Подожди и попробуй снова.", reply_markup=kb_payment(get_user_lang(m.from_user.id if m.from_user else 0)))

        events = _load_events()
        target = next((e for e in events if e["id"] == ev_id), None)
        if not target:
            await state.clear()
            return await m.answer("❌ Событие не найдено.", reply_markup=kb_main_for(m))

        if opt_type == "top":
            if not days:
                return await m.answer("❌ Не задан срок ТОП.", reply_markup=kb_top_duration(get_user_lang(m.from_user.id if m.from_user else 0)))
            target["is_top"] = True
            ...
            return await m.answer(
                ...
                reply_markup=kb_upsell_more(get_user_lang(m.from_user.id if m.from_user else 0))
            )
    ...
    await m.answer("Выбери пункт из меню.", reply_markup=kb_upsell(get_user_lang(m.from_user.id if m.from_user else 0)))


# ===================== БАННЕРЫ (АПСЕЛ) =====================

@dp.message(AddBanner.duration, F.text.in_(["⬅ Назад", "⬅ Back"]))
async def banner_duration(m: Message, state: FSMContext):
    if m.text in ["⬅ Назад", "⬅ Back"]:
        await state.set_state(AddEvent.upsell)
        return await m.answer("Выберите дополнительную опцию:", reply_markup=kb_upsell(get_user_lang(m.from_user.id if m.from_user else 0)))
...
async def banner_choose_duration(m: Message, state: FSMContext):
    if m.text not in BANNER_DURATIONS:
        return await m.answer("Выбери один из вариантов:", reply_markup=kb_banner_duration(get_user_lang(m.from_user.id if m.from_user else 0)))
...
async def banner_pay_link(m: Message, state: FSMContext):
    ...
    if not days:
        return await m.answer("❌ Срок не выбран.", reply_markup=kb_banner_duration(get_user_lang(m.from_user.id if m.from_user else 0)))
    ...
    for _, (d, a) in BANNER_DURATIONS.items():
        if d == days:
            ...
            break
    if amount is None:
        return await m.answer("❌ Тариф не найден.", reply_markup=kb_banner_duration(get_user_lang(m.from_user.id if m.from_user else 0)))
    ...
    if not link or not uuid:
        return await m.answer("⚠ Не удалось получить ссылку.", reply_markup=kb_payment(get_user_lang(m.from_user.id if m.from_user else 0)))
...
async def banner_paid(m: Message, state: FSMContext):
    ...
    if not uuid:
        return await m.answer("❌ Счёт не найден. Получи ссылку ещё раз.", reply_markup=kb_payment(get_user_lang(m.from_user.id if m.from_user else 0)))
    ...
    media = d.get("b_media")
    if not media:
        return await m.answer("❌ Медиа не найдено. Начни заново.", reply_markup=kb_main_for(m))
...
@dp.message(AddBanner.payment, F.text.in_(["⬅ Назад", "⬅ Back"]))
async def banner_pay_back(m: Message, state: FSMContext):
    await state.set_state(AddBanner.duration)
    await m.answer("🔙 Вернулись к выбору срока размещения баннера:", reply_markup=kb_banner_duration(get_user_lang(m.from_user.id if m.from_user else 0)))


# ===================== ПОИСК СОБЫТИЙ =====================

@dp.message(F.text.in_(["📍 Найти события рядом", "📍 Find events nearby"]))
...
async def search_menu_router(m: Message, state: FSMContext):
    ...
    if text in ["⬅ Назад", "⬅ Back"]:
        await state.clear()
        return await m.answer("Главное меню:", reply_markup=kb_main_for(m))
    ...
    if not category_filter:
        return await m.answer("Выбери один из вариантов:", reply_markup=kb_main_for(m))
...
async def show_favorites(m: Message):
    ...
    if not fav_ids:
        return await m.answer("У тебя пока нет избранных событий ⭐", reply_markup=kb_main_for(m))
...
async def on_startup():
    # Инициализируем SQLite, прежде чем стартовать вебхук
    try:
        init_db()
    except Exception as e:
        logging.warning(f"Не удалось инициализировать SQLite: {e}")
    if not PUBLIC_URL:
