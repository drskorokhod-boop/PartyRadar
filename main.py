...
async def show_nearby_banner_for_user(m: Message):
    """
    Показать один баннер при старте, если есть активный баннер в радиусе 30 км
    от последней геопозиции пользователя.
    """
    users = _load_users()
    ...
    # Один баннер — самый свежий (по id)
    candidates.sort(key=lambda x: x[0].get("id", 0), reverse=True)
    banner, _ = candidates[0]
    try:
        await send_banner(m.chat.id, banner)
    except Exception as e:
        logging.exception(f"Ошибка отправки баннера: {e}")
...
async def send_logo_then_welcome(m: Message):
    ...
    await m.answer(
        "⚡️ <b>PartyRadar</b>\n"
        "Твой радар по людям, встречам и движу рядом.\n\n"
        "📍 Ищи вечеринки, свидания, спорт, маркеты и работу по геолокации.\n"
        "📢 Создавай свои события и продвигай их через ТОП, Push и баннеры.\n\n"
        "📅 Дату, время и цену ты указываешь прямо в описании события.\n"
        "Отдельных шагов с датой больше нет — всё быстро и по делу.\n\n"
...
async def ev_title(m: Message, state: FSMContext):
    ...
    await m.answer(
        "🧾 Теперь введи <b>описание</b> события.\n\n"
        "Обязательно укажи в тексте:\n"
        "• дату и время (например, 25.10 в 19:30)\n"
        "• район / место\n"
        "• цену / стоимость участия (если она есть)\n"
        "• формат и важные условия (дресс-код, ограничения и т.п.)\n\n"
        "⚠️ Отдельного шага с датой нет — всё пишем здесь, в описании.",
        reply_markup=kb_back()
    )
...
async def ev_cat(m: Message, state: FSMContext):
    ...
    await m.answer(
        ...
        "• или сразу отправь геолокацию.\n\n"
        "❗ Если хочешь потом запускать <b>баннер</b> для этого события — медиа нужно загрузить <b>на этом шаге</b>.\n"
...
async def ev_media_other(m: Message, state: FSMContext):
    ...
    # а спокойно объясняем, что нужно сделать.
    await m.answer(
        "Сейчас мы на шаге медиа.\n"
...
async def banner_paid(m: Message, state: FSMContext):
    ...
    await state.clear()
    await m.answer("✅ Баннер активирован и будет показываться пользователям в твоём районе.", reply_markup=kb_main())
...
async def _search_and_show(m: Message, user_loc, category_filter, state: FSMContext):
    ...
    # Сначала обычные, потом ТОП — пользователь увидит ТОПы последними сообщениями
