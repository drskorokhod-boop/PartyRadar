...
async def send_banner(chat_id: int, b: dict):
    ...
    # Поддерживаем оба формата хранения:
    # 1) старый: media_type/file_id/is_local на верхнем уровне
    # 2) новый: media = {"type", "file_id", "is_local"}
    ...
    media = b.get("media")
    if media:
        media_type = media.get("type", media_type)
        file_id = media.get("file_id", file_id)
        is_local = media.get("is_local", is_local)
