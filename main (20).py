...
async def ev_lifetime(m: Message, state: FSMContext):
    ...
    if hours == 24:
        ...
        # Предлагаем платные опции
        await state.set_state(AddEvent.upsell)
        return await m.answer(
            "💡 Дополнительные опции продвижения:\n"
            "⭐ ТОП — событие будет выше остальных в своей категории.\n"
            "📣 Push — уведомление пользователям в радиусе 30 км.\n"
            "🖼 Баннер — крупный баннер после приветствия у всех пользователей.\n\n"
            "Выбери опцию или оставь объявление без продвижения.",
...
async def ev_pay_check(m: Message, state: FSMContext):
    ...
    await m.answer(
        ...
        "💡 Также доступно продвижение объявления:",
        reply_markup=kb_upsell()
    )
