...
def _save_events(data: List[dict]):
    """
    Безопасное сохранение списка событий в SQL.

    Вместо полного очищения таблицы:
    - обновляем/добавляем события по их id (upsert),
    - при необходимости удаляем только те записи, которых больше нет в data.

    Это снижает риск потери всех событий при сбое во время записи.
    """
    with get_session() as session:
        # Текущие id в базе
        existing_ids = {row.id for row in session.query(EventRow.id).all()}
        new_ids = set()
        ...
        for ev in data:
            ...
            try:
                ...
                continue
            new_ids.add(ev_id_int)
            session.merge(EventRow(id=ev_id_int, payload=ev))

        # Удаляем только те записи, которых нет в новом списке
        obsolete_ids = existing_ids - new_ids
        if obsolete_ids:
            session.query(EventRow).filter(EventRow.id.in_(obsolete_ids)).delete(synchronize_session=False)
...
async def ev_lifetime(m: Message, state: FSMContext):
    ...
    if hours == 24:
        ...
        # После любой публикации полностью сбрасываем состояние и переводим в upsell
        await state.clear()
...
async def ev_pay_check(m: Message, state: FSMContext):
    ...
    if already_published:
        # На всякий случай переводим пользователя в upsell-меню
        await state.clear()
        await state.set_state(AddEvent.upsell)
        return await m.answer(
...
"
    ...
    # После публикации очищаем состояние и переводим в upsell
    await state.clear()
    await state.set_state(AddEvent.upsell)
    await m.answer(
...
"
    ...
    await state.update_data(banner_done=True, _pay_uuid=None)
