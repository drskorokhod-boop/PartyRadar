...
PUSH_PRICE_USD = 1.5  # цена одного запуска Push-рассылки в USD

# Сроки баннеров
BANNER_DURATIONS = {
    "📅 1 день — $7": (1, PRICES["banner_1d"]),
    "📅 3 дня — $15": (3, PRICES["banner_3d"]),
    "📅 7 дней — $30": (7, PRICES["banner_7d"]),
    "📅 15 дней — $45": (15, PRICES["banner_15d"]),
    "📅 30 дней — $75": (30, PRICES["banner_30d"]),
}
...
async def upsell_push(m: Message, state: FSMContext):
    ...
    await m.answer(
        "📣 <b>Push-рассылка</b> — отправка объявления в личные сообщения всем активным пользователям в радиусе 30 км от точки события.\n\n"
        f"💵 Стоимость одного запуска: <b>${PUSH_PRICE_USD}</b>\n\n"
        "После оплаты мы разошлём это событие всем пользователям поблизости. Нажми «💳 Получить ссылку на оплату».",
...
async def banner_duration(m: Message, state: FSMContext):
    ...
    desc = (
        ...
        "Баннер показывается пользователям в том регионе, который вы выбрали при создании события "
        "(в ротации до 3 шт. после приветствия).\n\n"
...
async def send_event_media(chat_id: int, ev: dict, with_distance: Optional[float] = None):
    ...
    if ev.get("id") is not None:
        ...
        # если это автор события — добавим кнопку удаления
...
async def send_logo_then_welcome(m: Message):
    ...
    welcome_text = (
        ...
        "• Локальная реклама по геолокации — показ баннеров и событий людям рядом.\n"
        "• Продвижение офферов через ТОП и баннеры без сложной настройки.\n\n"
    ...
    # показать до 3 активных баннеров в регионе пользователя (если известна его последняя геолокация)
    banners = _load_banners()
    now = datetime.now()
    ...
    for b in banners:
        ...
        b_lat = b.get("lat")
        b_lon = b.get("lon")
        # если у баннера нет гео или у пользователя нет гео — не показываем, чтобы сохранить логику "по региону"
        if b_lat is None or b_lon is None or u_lat is None or u_lon is None:
            continue
        try:
            from geopy.distance import geodesic
...
async def ev_media_back(m: Message, state: FSMContext):
    ...
    # Если уже есть медиа — удаляем последнее
    if files:
        files.pop()
        await state.update_data(media_files=files)
        return await m.answer(f"🗑 Удалён последний файл ({len(files)}/3).", reply_markup=kb_media_step())

    # Если медиа ещё нет — возвращаемся в предыдущий логический шаг
    # Для категорий без даты (работа, покажи себя, ищу тебя, поздравления) → обратно к выбору категории
    ...
    # Для остальных → обратно к вводу даты/времени
    await state.set_state(AddEvent.dt)
...
async def ev_lifetime(m: Message, state: FSMContext):
    ...
    # 1 день — бесплатное размещение
    if hours == 24:
        await publish_event(m, data, hours)

        # Показываем, как выглядит объявление
        events = _load_events()
        if events:
            ev = events[-1]
            try:
        ...
        await state.clear()
        return await m.answer(
            "✅ Событие опубликовано бесплатно на 1 день.\n\n"
            "📤 Ты можешь поделиться объявлением в любые чаты или каналы — просто нажми стрелку «Переслать» под сообщением.",
            reply_markup=kb_main()
        )
    ...
    desc = (
        ...
        "Дополнительно можно включить платные опции (ТОП, баннеры, пуши) после оплаты.\n\n"
...
async def ev_upsell(m: Message, state: FSMContext):
    ...
    if txt == "🌍 Разместить бесплатно (без опций)":
        # Показываем последнее событие пользователя (оно уже опубликовано)
        events = _load_events()
        if events:
            ev = events[-1]
            try:
...
async def ev_opt_paid(m: Message, state: FSMContext):
    ...
    if opt_type == "top":
        ...
        # фиксируем момент активации ТОП, чтобы сортировать ТОП-объявления по дате оплаты
...
async def cb_delete_event(cq: CallbackQuery):
    try:
        _prefix, ev_id_str = cq.data.split(":", 1)
        ev_id = int(ev_id_str)
    except Exception:
...
async def _search_and_show(m: Message, user_loc, category_filter, state: FSMContext):
    ...
    # сначала ТОП-объявления (последние оплаченные выше), затем обычные по расстоянию
    def _sort_key(item):
        ev, dist = item
        is_top = ev.get("is_top")
        if is_top:
            paid_dt = _safe_dt(ev.get("top_paid_at")) or _safe_dt(ev.get("created")) or datetime.min
            # минус timestamp, чтобы самые свежие ТОП были выше
