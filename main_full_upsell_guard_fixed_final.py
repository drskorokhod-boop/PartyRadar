...
async def ev_lifetime(m: Message, state: FSMContext):
    ...
    if hours == 24:
        ...
        # После публикации полностью очищаем состояние и переводим пользователя в upsell
        await state.clear()
...
async def ev_pay_check(m: Message, state: FSMContext):
    ...
    if already_published:
        # На всякий случай очищаем состояние и переводим пользователя в upsell
        await state.clear()
        await state.set_state(AddEvent.upsell)
        return await m.answer(
...
async def banner_paid(m: Message, state: FSMContext):
    ...
    # Достаём данные баннера из состояния
    data = await state.get_data()
