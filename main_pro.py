...
def get_user_lang(user_id: int) -> str:
    ...
    if lang in (LANG_RU, LANG_EN):
        return lang
    return LANG_RU  # default Russian
...
def kb_media_step(lang: str = LANG_RU):
    if lang == LANG_EN:
        return ReplyKeyboardMarkup(
            keyboard=[[KeyboardButton(text="⬅ Back")]],
            resize_keyboard=True
...
def kb_categories(lang: str = LANG_RU):
    if lang == LANG_EN:
        return ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="🎉 Party"), KeyboardButton(text="💬 Date")],
                [KeyboardButton(text="🧠 Meetup"), KeyboardButton(text="⚽ Activity/Sport")],
                [KeyboardButton(text="🛒 Buy"), KeyboardButton(text="💰 Sell")],
                [KeyboardButton(text="💼 Looking for a job"), KeyboardButton(text="🧑‍💼 Hiring")],
                [KeyboardButton(text="✨ Show yourself"), KeyboardButton(text="🔍 Looking for you")],
                [KeyboardButton(text="🎊 Greetings"), KeyboardButton(text="🧭 Other")],
...
def kb_lifetime(lang: str = LANG_RU):
    if lang == LANG_EN:
        return ReplyKeyboardMarkup(
            keyboard=[
...
def kb_payment(lang: str = LANG_RU):
    if lang == LANG_EN:
        return ReplyKeyboardMarkup(
            keyboard=[
...
def kb_payment_method(lang: str = LANG_RU):
    if lang == LANG_EN:
        return ReplyKeyboardMarkup(
            keyboard=[
...
def kb_upsell(lang: str = LANG_RU):
    if lang == LANG_EN:
        return ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="⭐ TOP promotion")],
                [KeyboardButton(text="📣 Push (30 km radius)")],
...
def kb_upsell_more(lang: str = LANG_RU):
    if lang == LANG_EN:
        return ReplyKeyboardMarkup(
            keyboard=[
...
def kb_top_duration(lang: str = LANG_RU):
    if lang == LANG_EN:
        return ReplyKeyboardMarkup(
            keyboard=[
...
def kb_banner_duration(lang: str = LANG_RU):
    if lang == LANG_EN:
        return ReplyKeyboardMarkup(
            keyboard=[
...
async def send_logo_then_welcome(m: Message, lang: str):
    logo_path = None
    ...
    await m.answer(text, reply_markup=kb_main(lang))
...
@dp.message(F.text.in_(LANG_CHOICES.keys()))
async def choose_lang(m: Message, state: FSMContext):
    lang = LANG_CHOICES.get(m.text, LANG_RU)
...
async def support(m: Message):
    await m.answer(
        ...
        reply_markup=kb_main(get_user_lang(m.from_user.id))
    )
...
async def ev_title(m: Message, state: FSMContext):
    if m.text in ("⬅ Назад", "⬅ Back"):
        await state.clear()
        return await m.answer("Окей, вернулись в главное меню 👋", reply_markup=kb_main(get_user_lang(m.from_user.id)))
...
MARKET_CATS = ("🛒 Куплю", "💰 Продам", "🛒 Buy", "💰 Sell")
...
async def ev_cat(m: Message, state: FSMContext):
    ...
    # Работа
    if cat in ("💼 Ищу работу", "🧑‍💼 Предлагаю работу", "💼 Looking for a job", "🧑‍💼 Hiring"):
...
@dp.message(AddEvent.payment, F.text.in_({"✅ Я оплатил", "✅ I have paid"}))
...
async def ev_upsell(m: Message, state: FSMContext):
    ...
    if txt == "⬅ Назад":
        await state.clear()
        return await m.answer("Главное меню:", reply_markup=kb_main(get_user_lang(m.from_user.id)))
    ...
    # Push
    if txt in ("📣 Push-рассылка (30 км)", "📣 Push (30 km radius)"):
        events = _load_events()
        user_events = [e for e in events if int(e.get("author", 0)) == int(m.from_user.id)]
        if not user_events:
            await state.clear()
            return await m.answer("❌ У тебя пока нет опубликованных событий.", reply_markup=kb_main(get_user_lang(m.from_user.id)))
   ...
   # Баннер
    if txt in ("🖼 Баннер (премиум)", "🖼 Banner (premium)"):
        ...
        if not user_events:
            await state.clear()
            return await m.answer("❌ У тебя пока нет событий для баннера.", reply_markup=kb_main(get_user_lang(m.from_user.id)))
...
async def ev_upsell_more(m: Message, state: FSMContext):
    txt = m.text or ""
    if txt in ("➕ Добавить ещё опцию", "➕ Add another option"):
        await state.set_state(AddEvent.upsell)
        return await m.answer("Выберите дополнительную опцию для этого объявления:", reply_markup=kb_upsell())

    if txt in ("🏠 Главное меню", "Главное меню", "🏠 Main menu", "Main menu"):
        await state.clear()
        return await m.answer("Возврат в главное меню 👇", reply_markup=kb_main(get_user_lang(m.from_user.id)))
...
async def ev_opt_router(m: Message, state: FSMContext):
    ...
    if txt.startswith("⭐ "):
        ...
        if not user_events:
            await state.clear()
            return await m.answer("❌ У тебя нет событий для ТОП.", reply_markup=kb_main(get_user_lang(m.from_user.id)))
    ...
    if txt == "✅ Я оплатил":
        ...
        if not target:
            await state.clear()
            return await m.answer("❌ Событие не найдено.", reply_markup=kb_main(get_user_lang(m.from_user.id)))
...
async def banner_paid(m: Message, state: FSMContext):
    ...
    media = d.get("b_media")
    if not media:
        return await m.answer("❌ Медиа не найдено. Начни заново.", reply_markup=kb_main(get_user_lang(m.from_user.id)))
...
async def search_menu_router(m: Message, state: FSMContext):
    ...
    if not category_filter:
        return await m.answer("Выбери один из вариантов:", reply_markup=kb_main(get_user_lang(m.from_user.id)))
...
async def _search_and_show(m: Message, user_loc, category_filter, state: FSMContext):
    ...
    for ev in events:
        ...
        if category_filter == "market" and cat not in ("🛒 Куплю", "💰 Продам"):
            continue
        if category_filter == "work" and cat not in ("💼 Ищу работу", "🧑‍💼 Предлагаю работу", "💼 Looking for a job", "🧑‍💼 Hiring"):
            continue
...
async def show_favorites(m: Message):
    ...
    if not fav_ids:
        return await m.answer("У тебя пока нет избранных событий ⭐", reply_markup=kb_main(get_user_lang(m.from_user.id)))
