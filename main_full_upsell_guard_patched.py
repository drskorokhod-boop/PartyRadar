...
def _save_events(data: List[dict]):
    """
    Полная синхронизация списка событий в SQL с безопасной UPSERT-логикой.

    Вместо полного удаления всех записей:
...
async def ev_lifetime(m: Message, state: FSMContext):
    ...
    if hours == 24:
        ...
        # После публикации очищаем состояние и переводим пользователя в upsell
        await state.clear()
