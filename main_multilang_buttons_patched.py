...
async def ev_lifetime(m: Message, state: FSMContext):
    if m.text in ["⬅ Назад", "⬅ Back"]:
        await state.set_state(AddEvent.contact)
        return await m.answer("☎ Укажи контакт или напиши «Пропустить».", reply_markup=kb_back(get_user_lang(m.from_user.id if m.from_user else 0)))

    if m.text not in LIFETIME_OPTIONS:
        return await m.answer("Выбери срок из списка:", reply_markup=kb_lifetime(get_user_lang(m.from_user.id if m.from_user else 0)))
